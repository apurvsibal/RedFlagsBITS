...
from moderation import ChatModeration
    ...
    if session['username']!= None:
        if request.method == 'POST':
            ...
            if ChatModeration(title):
                flash("Please give an appropriate title in accordance with the community guidlines.")
                return render_template("create_posts.html", title = title, content = content)
            if ChatModeration(content):
                flash("Please keep the content in accordance with the community guidlines.")
                return render_template("create_posts.html", title = title, content = content)
            conn = sqlite3.connect('users.db')
            cursor = conn.cursor()
