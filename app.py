...
def sendMail(toEmail, subject, content):
    # The below credentials are for the senders email
    ...
    # Generates the content of the email
    email = MIMEMultipart()
    ...
    if request.method == 'GET':
        ...
        if not username:
            ...
            if row is not None:
                ...
                conn.commit()
                conn.close()
                reset_link = "http://127.0.0.1:8000/reset_password?token=" + reset_token
    ...
    if request.method == 'GET':
        ...
        if not password:
            ...
            if row is not None:
                ...
                flash("Error while resetting password")
                return render_template('forgotpassword.html')
